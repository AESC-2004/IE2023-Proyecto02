...
def escalar_a_255(valor_str):
    ...
    valor_escalado = int((valor_float / 180.0) * 255.0)
...
ADAFRUIT_IO_KEY = "aio_hocI80J7ooQDxqLaXMx03DaNeKqB"

# Set to the ID of the feed to subscribe to for updates.
FEED_ID_receive = 'Motor1_TX'
...
def message(client, feed_id, payload):
    ...
    # if (payload == '1'):
    #   arduino.write(bytes('Turn ON LED', 'utf-8'))
    # elif (payload == '0'):
    #    arduino.write(bytes('Turn OFF LED', 'utf-8'))
    # Publish or "send" message to corresponding feed
    print('Sendind data back: {0}'.format(payload))
    client.publish(FEED_ID_Send, payload)
    # Mapping the stablished grades into a readable value for the MCU
    valor_255 = escalar_a_255(payload)
    mensaje_uart = f'#A:{valor_255};'
    arduino.write(bytes(mensaje_uart, 'utf-8'))
    time.sleep(0.5)
